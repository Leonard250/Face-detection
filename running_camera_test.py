...
model = YOLOv3(in_channels=3, num_classes=21).to(device) 
...
while True:
    ...
    ret, frame = cap.read()
    if not ret:
        print("Failled to capture")
        break

    f_resized = cv2.resize(frame, (416, 416))
    f_normalised = f_resized / 255.0
    f_tensor = torch.from_numpy(f_normalised).float().permute(2, 0, 1).unsqueeze(0).to(device)
    model.eval()
    with torch.no_grad(): 
    ...
    for i in range(batch_size):
        nms_boxes = nms(bboxes[i], iou_threshold= 0.4, threshold= 0.5)
        plot_image(f_tensor[i].permute(1, 2, 0).cpu(), nms_boxes)

    if cv2.waitKey(1) & 0xFF == ord('q'):
        break
ending_time  =  time.time()  
time_it_take = ending_time - start_time
print("time taken is ", time_it_take)
cap.release()
